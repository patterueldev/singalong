...
async def websocket_player_discovery_endpoint(websocket: WebSocket):
    ...
    **Protocol**:
    ...
    # Check if discovery is locked (in-memory; restored from DB on startup)
    if discovery_manager.is_discovery_locked():
        locked = next(
            (p for p in discovery_manager.get_all_players() if p.status == "locked"),
            None,
        )
        logger.info(
            f"[Discovery] rejecting_connection | reason=discovery_locked | "
            f"player_id={locked.player_id if locked else 'unknown'}"
        )
        await websocket.close(
            code=status.WS_1008_POLICY_VIOLATION,
